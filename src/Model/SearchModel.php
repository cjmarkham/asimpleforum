...
class SearchModel 
{
	...
	public function get ($query, $selection)
	{
		...
		$topic_sql = 'SELECT t.*, f.name as forumName, p.name as lastPostName, p.id as lastPostId, u.username as author, us.username as lastPosterUsername FROM topics t JOIN forums f ON f.id=t.forum JOIN users u ON t.author=u.id JOIN users us ON us.id=t.lastAuthorId LEFT JOIN posts p ON t.lastPostId=p.id WHERE t.name LIKE ? ORDER BY sticky DESC, updated DESC LIMIT 10';
